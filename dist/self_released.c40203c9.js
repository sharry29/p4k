...
var frame = d3.select('#chart-2').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).call(responsivefy);
var svg = frame.append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var bottom = frame.append('g').attr('transform', "translate(".concat(margin.left, ", ").concat(margin.top + height, ")"));
var xPositionScale = d3.scaleLinear().domain([0, 9]).range([0, width]); // .padding(0.01)

var yPositionScale = d3.scaleLinear().domain([0, 19]).range([0, height]); // .padding(0.01)
...
function ready(_ref) {
  ...
  var gs = svg.selectAll('.song').data(datapoints).enter().append('g');
  gs.append('circle') // .attr('cx', d => xPositionScale(Math.floor((+d.rank - 1) / 20)))
  .attr('cx', function (d) {
